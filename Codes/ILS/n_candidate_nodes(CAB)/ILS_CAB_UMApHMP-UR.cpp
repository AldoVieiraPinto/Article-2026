...
double floyd_classico(int n, int p, double alpha, int* s, int w, const Cenario cen[], int size){
	double** c = cria_matriz_double(n, n);
 	for(int i=0 ; i < p ; i++){
		for (int j=0; j<n; j++){
			if(j<p){
				c[s[i]][s[j]]=alpha*cen[w].custo[s[i]][s[j]];
			} 
			else{
	...
	for(int i=0 ; i < n ; i++){
		for (int j=0; j<n; j++){
			soma = soma + c[s[i]][s[j]] * cen[w].fluxo[s[i]][s[j]];
		}
	}
...
double FO(int n, int p, double alpha, double beta, int* s, const Cenario cen[], int size){
	...
	for (int t = 0; t<size; t++){
		custo_cen[t]=floyd_classico(n, p, alpha, s, t, cen, size);
		pp[t]=cen[t].prob;
	}
	double result;
	result = cvar(custo_cen, pp, beta, size);
	// if NEU:
	// result = media(custo_cen, pp, size);
	// if MCVaR:
	// result = mcvar(custo_cen, pp, beta, size);
	return result;
}
