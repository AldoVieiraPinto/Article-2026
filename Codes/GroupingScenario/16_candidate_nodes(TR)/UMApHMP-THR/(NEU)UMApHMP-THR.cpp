...
#define inf 10e10 /// Parametro usado para representar um n̼mero suficientemente grande
...
void DEP_grupo(int n, int p, double alpha, int numcen, int cenarios_por_grupo, int gr, const std::vector <int>& cenarios,
	...
	// ====================================Formulation=============================================================== 
	// minimize sum (m in 1..n) prob*custocenario[m]
	// ===============================================================================================================

	IloExpr expfo(env);
	for (int m = 0; m < numcen; m++) {
		expfo += prob * custocenario[m];
	}
...
int main(int argc, char* argv[]) {
	try {
		...
		for (int gr = 0; gr < total_grupos; gr++) {
			DEP_grupo(n, p, alpha, numcen, cenarios_por_grupo, gr, cenario, custo, fluxo, solucao, hubs_grupo, s);
			printf("\n\nGrupo %d : %10.4f \n\n", gr, solucao[gr]);
		}
		double soma = 0;
		...
		int indicepior;
		for (int gr = 0; gr < total_grupos; gr++) {
			auxiliar = DEP_todos(gr, n, p, alpha, numcen, custo, fluxo, hubs_grupo, s);
			if (auxiliar < ub ){
			      ub = auxiliar;
			      indice = gr;
			}
		...
		re3 = fopen("Tabela.txt","aw+");
                fprintf(re2, "\n\n %s - %d - %d - %d - %d - %d \t %10.4f \t %10.4f \t  %10.4f \t", argv[1], n, numcen , p, alpha0, tempo, ub, lb, gap_cen);
                fprintf(re3, "\n %s - %d - %d - %d - %d - %d \t %10.4f \t %10.4f \t  %10.4f \t ", argv[1], n, numcen , p, alpha0, tempo, ub, lb, gap_cen);
		for (int j = 0; j < n; j++) {
